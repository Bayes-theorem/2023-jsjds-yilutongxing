...
Component({
  ...
  lifetimes: {
    ...
    ready() {
      ...
      tim.on(timEvent.MESSAGE_RECEIVED, this.$onMessageReceived, this);
      tim.on(timEvent.MESSAGE_READ_BY_PEER, this.$onMessageReadByPeer, this);
      tim.on(timEvent.MESSAGE_REVOKED, this.$onMessageRevoked, this);
    },

    detached() {
      // 一定要解除相关的事件绑定
      tim.off(timEvent.MESSAGE_RECEIVED, this.$onMessageReceived);
      tim.off(timEvent.MESSAGE_READ_BY_PEER, this.$onMessageReadByPeer);
      tim.off(timEvent.MESSAGE_REVOKED, this.$onMessageRevoked);
    },
  },
  ...
  methods: {
    ...
    // 撤回消息重新发送
    resendMessage() {
      tim.resendMessage(this.data.resendMessage)
        .then((imResponse) => {
